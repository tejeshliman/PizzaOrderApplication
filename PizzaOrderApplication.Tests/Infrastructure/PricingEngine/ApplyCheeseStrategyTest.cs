...
{
    public class ApplyCheeseStrategyTest
    {
        private readonly IProductRepository _productRepository;
        private readonly IPricingStrategy _pricingStrategy;
        public ApplyCheeseStrategyTest()
        {
            _productRepository = A.Fake<IProductRepository>();
            _pricingStrategy = new ApplyCheeseStrategy(_productRepository);
        }
        ...
        {
            CartItem cartItem = new CartItem() { CartItemId = 1, ExtraChese = false, ProductSize = ProductSize.Small, Quantity = 1 };
            A.CallTo(() => _productRepository.GetProductPriceByCategoryAndType(A<Category>._, A<int>._)).Throws(new Exception("Error While Calculating price using ApplyCheeseStrategy"));
